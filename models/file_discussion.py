class FileDiscussion:
    ...
    def __init__(self, id_file_discussion, nom_discussione, date_creation):
...
class FileDiscussionRepository:
    ...
    def find_all(self):
        cur = self.db.cursor()
        cur.execute("SELECT id_file_discussion, nom_discussione, date_creation FROM file_discussion")
        rows = cur.fetchall()

        file_discussions = [FileDiscussion(r[0], r[1], r[2]) for r in rows]
