...
def supprimer_discussion(id_discussion):
    conn = db_manager.get_connection()
    repo = MessagerieRepository(conn)
    repo.delete_discussion(id_discussion)
    conn.close()
    flash("Le fil de discussion a été supprimé définitivement.", "success")
    return redirect(url_for('chat.boite_reception'))

@chat_bp.route('/messagerie/envoyer/<int:id_discussion>', methods=['POST'])
@role_required(['client', 'commercial'])
def envoyer_message(id_discussion):
    message_texte = request.form.get('contenu_message')
    if message_texte:
        conn = db_manager.get_connection()
        repo = MessagerieRepository(conn)
